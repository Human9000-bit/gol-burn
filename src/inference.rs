...
pub fn convert_to_string<B: Backend>(grid: Tensor<B, 4, Bool>) -> String {
    let dims = grid.dims();
    let grid = grid.reshape([dims[2], dims[3]]).to_data();
    let bool_matrix_vec: Vec<u8> = grid.to_vec().unwrap();
    let bool_matrix_vec: Vec<bool> = bool_matrix_vec.into_iter().map(|b| b != 0).collect();
